...
class Blog extends CI_Controller {
	...
	function delete($id = NULL){
		if($this->blog_model->delete_news($id)){
	...
	function add(){
		$user = $this->session->userdata('logged_in');
			...
			$result = $this->do_upload("image_news", $pathMain);
                ...
                {
                    $pathThumb = './upload/news';
		            if (!is_dir($pathThumb))
		                mkdir($pathThumb, 0755);

		            //in other folder
                    if($this->resize_image($pathMain . '/' . $result['upload_data']['file_name'], $pathThumb . '/'. md5($result['upload_data']['raw_name']).$result['upload_data']['file_ext'],'200','200')){
                    	if($this->resize_image($pathMain . '/' . $result['upload_data']['file_name'], $pathThumb . '/'."848x300". md5($result['upload_data']['raw_name']).$result['upload_data']['file_ext'],'848','300')){
	                        if($this->input->post('title') != null){
								$this->blog_model->add_news(md5($result['upload_data']['raw_name']).$result['upload_data']['file_ext'],$user['username'],now());
								redirect(base_url());
							}
						}
					}
